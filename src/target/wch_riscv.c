...
int wch_riscv_openocd_step(struct target *target, int current,	target_addr_t address, int handle_breakpoints);
...
int wch_riscv_add_breakpoint(struct target *target, struct breakpoint *breakpoint)
{
	...
	assert(breakpoint);
	if((riscvchip==6 ||(((uint16_t)chip_type) ==0x050c)||((chip_type) ==0x92000009)||(riscvchip==0x0c)||(riscvchip==0x0e)||(riscvchip==0x46)||(riscvchip==0x0f)||(riscvchip==0x86)||(riscvchip==0x4b)||(riscvchip==0x8e)) &&  target->breakpoints )	
		breakpoint->type = BKPT_HARD;

	if((riscvchip==6 ||(((uint16_t)chip_type) ==0x050c)||(riscvchip==0x0b && (chip_type !=0x92000009))||(riscvchip==0x0c)||(riscvchip==0x0e)) && breakpoint->type == BKPT_HARD && target->breakpoints ){
	...
	if(riscvchip==1||riscvchip==2||riscvchip==3 ||riscvchip==7 ||riscvchip==9  ||riscvchip==0x4E ||riscvchip==0x0a ||(((uint16_t)chip_type) ==0x0500)&&(riscvchip!=0x8e)){
			breakpoint->type = BKPT_SOFT;
		}
...
COMMAND_HANDLER(wch_riscv_reg_read)
{
	char *  regbuf[32]={"zero","ra","sp","gp","tp","t0","t1","t2","fp","s1","a0","a1","a2","a3","a4","a5",
	...
	if(strcmp("all",CMD_ARGV[0])==0){
		riscv_reg_t value;
		for(int i=0;i<32;i++){
			r->get_register(target,&value,i);
			command_print(CMD, " %s 0x%" PRIx32, regbuf[i],value);
		}
		return ERROR_OK;
	}
	uint32_t address;
	...
	COMMAND_PARSE_NUMBER(u32, CMD_ARGV[0], address);
	if(address>65)
		r->get_register(target,&value,address+65);
	else
		r->get_register(target,&value,address);
	command_print(CMD, "  0x%" PRIx32, value);
	return ERROR_OK;
}
...
static const struct command_registration riscv_exec_command_handlers[] = {
	...
	{
		.name = "reg_read",
		.handler = wch_riscv_reg_read,
		.mode = COMMAND_ANY,
		.usage = "address",
		.help = "Perform a 32-bit reg at address, returning the value."
	},
	{
		.name = "dmi_write",
